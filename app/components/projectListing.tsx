...
const ProjectListing = (props: IProject) => {
  const { title, projectImage, demoUrl, codeUrl, synopsis, technologies } =
    props;
    ...
    <li className='mb-12'>
      <div className='group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50'>
        ...
        <div className='z-10 sm:col-span-6'>
          ...
          <p className='mt-2 text-sm leading-normal'>{synopsis}</p>
          <ul className='mt-2 flex flex-wrap' aria-label='skills'>
            {technologies.map((technology: string) => {
              return <SkillPill skill={technology} />;
            })}
          </ul>
        </div>
      </div>
